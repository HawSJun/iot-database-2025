...
- MySQL 설치(Docker)
    ...
    4. Docker 컨테이너 생성
        ...
        - 컴퓨터 재시작 후 도커 컨테이너 자동 시작 옵션 명령어
        ```shell
...
- SQL 기초
    ...
    - DML 중 SELECT
        ...
        - 쿼리 연습(정렬까지) : [SQL](./day02/db02_select_query.sql)
...
- 정규화 : [SQL](./day06/db03_정규화_이상현상.sql)
    ...
    - 함수 종속성 - 하나의 속성(A)이 다른 속성(B)을 결정 지으면 A는 B의 결정자. A → B. A가 기본키가 됨
        - 완전함수종속 - 종속성에 일치하지 않은 속성이 하나도 없는 경우
    ...
    - 정규화 - 이상현상이 발생하지 않도록 릴레이션(개체, 테이블)을 분해하는 과정
        ...
        - BCNF정규화 - 함수종속성 A → B가 성립 시 모든 결정자 A가 후보키가 되는 정규화(무손실 분해)
        - 제4정규화 - 실무에서 사용 안함. 다치종속성 릴레이션 가진 정규화
        - 제5정규화 - 실무에서 사용 안함. 프로젝트-조인 정규형. 조인종속성을 가진 릴레이션

- 트랜잭션 : [SQL1](./day06/db04_트랜잭션.sql) / [SQL2](./day06/db05_트랜잭션2.sql)
    - 데이터를 다루는 논리적인 작업 단위
    - START TRANSACTION, SAVEPOINT, ROLLBACK [TO SAVEPOINT], COMMIT 트랜잭션 처리
    - 특징
        - A (원자성) - Atomicity, 원자처럼 쪼개지지 않고 한 덩어리로 취급, All or nothing.
        - B(일관성) - Consistency, 트랜잭션 전후의 데이터가 일관되게 저장되어 있어야 함.
...
## 7일차
- Workbench Tip
    - SQL 툴 공통으로 SELECT 실행 시 모든 행을 다 표시하시 않음.(성능저하 대비)
