...
SELECT custid
	 , COUNT(*) AS 구매도서수량  -- 함수의 매개변수로  *, custid
...
-- 외부 조인 : 조건을 만족하지 않는(일치하지 않는) 데이터도 출력이 필요할 때 사용하는 조인
-- 3-27 : 도서를 구매하지 않는 고객을 포함해 고객의 이름과 고객이 주문한 도서의 판매가격을 구하시오.
...
-- 3-31 : 출판사별로 출판사 평균도서가격보다 비싼 도서를 구하시오.
-- 상관 서브쿼리.
SELECT *
  FROM Book AS b1
 WHERE b1.price > ( 
