...
SELECT ROUND(4.875) AS 결과; -- 5
SELECT ROUND(4.875, 1) AS 결과; -- 4.9
...
-- DB는 인덱스룰 1부터 시작!!! (프로그래밍언어와 차이점)
SELECT SUBSTR('Hello MySQL', 7, 2);  -- 연습이 많이 필요
...
-- 4-7 마당서점은 주문일부터 10일 후에 매출을 확정합니다. 각 주무의 확정일자를 구하시오.
...
-- %Y = 년도 전체, %y = 년도 뒤 2자리, %M = July(월 이름), %b = Jul(월 약어), %m = 07(월 숫자)
-- %d = 일, %H = 16(오후), %h = 04(오후), %W = Monday, %w = 1(요일은 일요일 0)
-- %p = AM/PM
SELECT orderid AS '주문번호'
...
SELECT *
  FROM MyBook
 WHERE price IS NULL; -- 반재는 IS NOT NULL
