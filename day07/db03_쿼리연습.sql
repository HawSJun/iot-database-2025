...
/* 문제1 - employee 에서 사원의 성과 이름을 Name, 업무는 Job, 급여는 Salary, 연봉에 $100 보너스를 추가해서 계산한 Increased Ann_Salary
		   급여에 $100 보너스를 추가해서 계산한 연봉은 Increased Salary 별칭으로 출력하시오 (107행)
*/
...
/* 문제2 - employee에서 모든 사원의 last_name과 연봉을 '이름: 1 Year Salary = $연봉' 형식으로 출력하고, 1 Year Salary라는 별칭을 붙여 출력하시오.(107행)
*/
...
-- where, order by
/* 샘플 - hr부서 예산 편성문제로 급여 정보 보고서를 작성한다. employees 에서 salary가 7000 ~ 10000달러 범위 이외의 사람의
...
/* 문제2 - 현재 날짜 타입을 날짜 함수를 통해 확인, 1995년 5월 20일 ~ 1996년 5월 20일 사이 입사한 사원의 이름 Name으로 별칭
...
/* 문제1 - 사원이 소속된 부서별 급여 합계, 급여 평균, 급여 최대값, 급여 최소값을 집계.
           ...
           단, 부서에 소속되지 않는 사원은 정보에서 제외, 출력 시 머리글은 아래처럼 별칭으로 처리(11행)
*/
select department_id
	 , concat('$', format(round(sum(salary), 0), 0)) as 'Sum Salary'
	 , concat('$', format(round(avg(salary), 1), 1)) as 'AVG Salary'  -- round(값, 1), 소수점 1자리에서 반올림, format(값, 1) 소수점 표현 및 1000단위 ,표시
...
/* 문제3 - 각 사원의 상사와의 관게를 이용, 보고서 작성을 하려고 함
		   예를 보고 출력하시오.(107행)
*/
