...
/* 샘플문제1 - 프로젝트 성공으로 급여인상이 결정됨.
			   사원 현재 107명 19개 업무에 소속되어 근무중. 회사 업무 Distince job_id 5개 업무에서 일하는 사원.
...
select employee_id
     ...
     , job_id
     , salary
     , case job_id when 'HR-REP' then salary * 1.10
...
/* 샘플 - 부서와 업무별 급여 합계를 구하고 신년도 급여수준 레벨을 지정하려고 함.
		  부서 번호와 업무를 기준으로 전체 행을 그룹별로 나누어 급여 합계와 인원수를 출력(20개행)
*/
...
-- RANK
/* 샘플 - 분석함수 NTILE() 사용, 부서별 급여 합계를 구하시오.
		  급여가 제일 큰 것이 1, 제일 작은 것이 4가 되도록 등급을 나눔(12행)
*/
select department_id
	 , sum(salary) as 'Sum Salary'
     , ntile (3) over (order by sum(salary) desc) as 'Bucket'  -- 범위별로 등급 매기는 키워드
  from employees
 group by department_id;
 
/* 문제1 - 부서별 급여를 기준으로 내림차순으로 정렬하시오.
