...
def showDatas():
    ...
    for i, (std_id, std_name, std_mobile, std_regyear) in enumerate(data, start=1): # enumerate - 인덱스와 아이템 동시접근
        dataView.insert('', 'end', values=(std_id, std_name, std_mobile, std_regyear)) # 트리뷰 마다 'end' 추가

    cursor.close()  # 커서 종료
    conn.close()    # 커넥션 종료

# 6. getData(event) - 트리뷰 한 줄 더블 클릭한 값 엔트리에 표시
def getData(event):
    '''
    ...
    ent4.delete(0, END)  # 입학녀도 기존 데이터 삭제
    ...
    ## 엔트리 위젯 각각 채워넣기
    ent1.config(state='normal')     # 데이터들어갈 수 있게 해주고
...
def addData():
    ...
    try:
        ...
        print(e)
        conn.rollbakc() # 트랜잭션 롤백
        messagebox.showerror('INSERT', '학생 등록 실패!!')
    finally:
        cursor.close()  # 커서
        conn.close()    # 커넥션 종료
...
def modData():
    ...
    try:
        ...
        messagebox.showinfo('UPDATE', '학생 수정 성공!')
        ...
        print(e)
        conn.rollbakc() # 트랜잭션 롤백
        messagebox.showerror('UPDATE', '학생 수정 실패!!')
    finally:
        cursor.close()  # 커서
        conn.close()    # 커넥션 종료
...
def delData():
    ...
    try:
        ...
        messagebox.showinfo('DELETE', '학생 삭제 성공!')
        ...
        print(e)
        conn.rollbakc() # 트랜잭션 롤백
        messagebox.showerror('DELETE', '학생 삭제 실패!!')
    finally:
        cursor.close()  # 커서
        conn.close()    # 커넥션 종료
...
## 4-5 트리뷰 설정
for col in cols:
    dataView.heading(col, text=col)                 # 각 열 제목을 cols 변수 하니씩 지정
...
# 6. 트리뷰 항목 더블킬릭하면 이벤트 발샌(getData() 함수 호출)
